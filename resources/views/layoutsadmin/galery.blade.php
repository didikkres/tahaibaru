                        ...
                        <div class="body table-responsive">
                            <table class="table table-bordered table-striped table-hover js-basic-example dataTable" id="DataTables_Table_0" role="grid" aria-describedby="DataTables_Table_0_info">
                                ...
                                <tbody>
                                    ...
                                    <tr>
                                        <th scope="row">{{ $no++ }}</th>
                                        <td><a href="{{url('/storage/galery')}}/{{$b->gambar}}" data-sub-html="Demo Description">
                                        <img class="img-responsive thumbnail" src="{{url('/storage/galery') }}/{{$b->gambar}}">
                                    </a></td>
                                        <td>{{$b->judul}}</td>
                                        <td>{{$b->isi}}</td>
                                        <td>
                                <a href="/galeryadminedit/{{$b->id}}"  class="btn bg-yellow btn-circle waves-effect waves-circle waves-float"><i class="material-icons">edit</i></a>
    ...
    <div class="modal-dialog modal-xs" role="document">
        <div class="modal-content">
                <div class="modal-header">
                    <h4 class="modal-title" id="largeModalLabel">Tambah Galery</h4>
                </div>
                        <div class="modal-body">
                            <form action="/galeryadminpost" method="POST" enctype="multipart/form-data">
                            {{csrf_field()}}
...
@section('footer')
	<script >
		$('.delete').click(function(){
			...
			.then((willDelete) => {
				if (willDelete) {
					window.location = "galeryadmin/"+pendidikan_id+"/delete"
				}
			});
		});
	</script>
